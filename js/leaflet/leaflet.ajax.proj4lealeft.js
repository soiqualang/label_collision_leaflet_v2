...
L.GeoJSON.AJAX_PROJ4LEALEFT = L.GeoJSON.extend({
  defaultAJAXparams: {
    dataType: 'json',
  ...
  initialize: function (url,crs, options) {
    this.urls = [];
    if (url) {
  ...
  //Viết lại hàm add data (lấy ý tưởng từ Proj4Leaflet) để chuyển hệ tọa độ
...
L.geoJson.ajax_proj4lealeft = function (geojson,crs, options) {
  return new L.GeoJSON.AJAX_PROJ4LEALEFT(geojson,crs, options);
};
