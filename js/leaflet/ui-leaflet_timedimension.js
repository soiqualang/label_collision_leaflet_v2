...
angular.module('ui-leaflet', ['nemLogging']).directive('leaflet', ["$q", "leafletData", "leafletMapDefaults", "leafletHelpers", "leafletMapEvents", function ($q, leafletData, leafletMapDefaults, leafletHelpers, leafletMapEvents) {
    return {
        ...
        link: function link(scope, element, attrs, ctrl) {
            ...
            //Chỉnh map option để add vào angular

			var map_opt=leafletMapDefaults.getMapCreationDefaults(attrs.id);
			map_opt.zoomDelta=0.5;
			map_opt.zoomSnap= 0.5;   
			map_opt.fullscreenControl= true;
			map_opt.timeDimension= true;
            map_opt.timeDimensionControl= true;
            map_opt.timeDimensionControlOptions= {
                position: 'bottomleft',
                autoPlay: false,
                timeSlider: true,
                loopButton: true,
			...
			//console.table(map_opt);
			
            // Create the Leaflet Map Object with the options
            var map = new L.Map(element[0], map_opt);
